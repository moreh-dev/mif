...
- The decode proxy is launched with `--response-header`, so it injects `X-Decoder-Host-Port` and `X-Prefiller-Host-Port` headers without requiring Heimdall's `response-header-handler` plugin.
