...
- If using `pd-profile-handler` (or the newer `disagg-profile-handler`), you **must** also instantiate `prefill-filter`, `decode-filter`, one PD decider plugin (`prefix-based-pd-decider` by default for cache-aware PD, or `always-disagg-pd-decider` to force disaggregation on every request), and `disagg-headers-handler` (or its legacy alias `prefill-header-handler`). The filters must be present in the top-level `plugins` list because `schedulingProfiles[].plugins[].pluginRef` resolves against that list. The ordering requirement is specific to the decider and `disagg-headers-handler`: both must appear **before** the profile handler in the top-level `plugins` list, because the profile handler's factory looks them up during initialization.
