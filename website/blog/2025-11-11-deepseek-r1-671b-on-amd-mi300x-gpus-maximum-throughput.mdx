...
Please make sure to install all [prerequisites](/docs/getting-started/prerequisites) before starting this benchmarking. Also, please refer to the [quickstart](/docs/getting-started/quickstart) to understand how to run the MoAI Inference Framework.
...
**AWS credentials must be configured in this namespace to allow the container images of the MoAI Inference Framework to be downloaded**. For details, refer to the "Amazon ECR token for Moreh's container image repository" section in the [prerequisites](/docs/getting-started/prerequisites).
...
Inside the pod, you can run `vllm bench serve`. **You may need to modify the `--host` value depending on your Istio gateway address**. The following are the actual commands used to run each experiment. For each experiment, the warm-up time and cool-down time were adjusted appropriately.
...
The following are some publicly available performance numbers for comparison.
