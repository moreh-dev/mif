...
Odin is the operator that manages inference services — the vLLM pods that serve a model. Install it after cert-manager.

Install the Odin CRDs first. CRDs are packaged as a separate chart so their lifecycle is independent of the operator.

```shell
helm upgrade -i odin-crd moreh/odin-crd --version v0.12.0 -n mif
```

This installs the `InferenceService` and `InferenceServiceTemplate` custom resource definitions.
...
Heimdall is the operator that runs the AI Gateway. It reconciles `AIGateway` resources into a gateway deployment and injects the gateway sidecar into inference pods. Because it injects sidecars into Odin-managed pods, install it after Odin.

Install the Heimdall CRDs first.

```shell
helm upgrade -i heimdall-crd moreh/heimdall-crd --version v0.5.0 -n mif
helm upgrade -i heimdall-aigateway-crd moreh/heimdall-aigateway-crd --version v0.10.0 -n mif
```

This installs the `AIGateway` and `SchedulingProfile` custom resource definitions.
