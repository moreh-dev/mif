...
```mermaid
graph TB
    request["aigateway.request<br>(root span, request received)"]
    scheduler["aigateway.scheduler.run<br>(scheduling decision: scores candidate pods, picks one)"]
    upstream["aigateway.upstream.tito<br>(call to the selected vLLM worker)"]
