...
graph TB
    ...
    sp_llama["SchedulingProfile<br>llama"] -. "meta-llama/Llama-3.2-1B-Instruct" .-> gateway
    sp_qwen["SchedulingProfile<br>qwen"] -. "Qwen/Qwen3-1.7B" .-> gateway
    subgraph isvc_llama["Llama 3.2 1B"]
      llama_mi250["vLLM (MI250)"]
      llama_mi300x["vLLM (MI300X)"]
    end
    subgraph isvc_qwen["Qwen 3 1.7B"]
      qwen_mi250["vLLM (MI250)"]
      qwen_mi300x["vLLM (MI300X)"]
    end
    gateway -->|"meta-llama/Llama-3.2-1B-Instruct"| llama_mi250
    gateway -->|"meta-llama/Llama-3.2-1B-Instruct"| llama_mi300x
    gateway -->|"Qwen/Qwen3-1.7B"| qwen_mi250
    gateway -->|"Qwen/Qwen3-1.7B"| qwen_mi300x
```
...
- Complete the [Prerequisites](../getting-started/prerequisites.mdx) setup, including the MoAI Inference Framework installation. This installs the Odin and Heimdall operators this guide builds on.
- Confirm with your cluster administrator that the presets used in this guide are installed in the `mif` namespace. See [Presets](../features/preset.mdx).
