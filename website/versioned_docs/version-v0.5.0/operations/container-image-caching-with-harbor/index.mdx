...
Decide whether to configure Harbor over HTTP (insecure but simple) or HTTPS, depending on your environment. Then, deploy Harbor using the following command, which exposes Harbor over HTTP on port 80. **You need to replace `<password>` and `<storageClass>` with your own values.**
...
If you choose HTTPS, replace the `externalURL` and `expose.tls.enabled` flags above with the following. The `externalURL` scheme must match the scheme Harbor is exposed with, and `expose.tls.secret.secretName` refers to the `harbor-tls` certificate created in the HTTPS tab below.

```shell
    --set externalURL https://harbor.harbor.svc.cluster.local \
...
server = "http://harbor.harbor.svc.cluster.local:80"
