...
The `mif.moreh.io/aigateway` label binds the InferenceService to an `AIGateway` named `mif`. Odin propagates the label to the vLLM pods, and the Heimdall operator injects the gateway sidecar into them so the gateway can route requests to these pods. See the [Quickstart](../getting-started/quickstart.mdx) for the full deployment flow.
...
You can turn the configuration above into a reusable preset (`InferenceServiceTemplate`) by removing the `replicas` and `templateRefs` fields and the `mif.moreh.io/aigateway` label, then changing the `kind` to `InferenceServiceTemplate`. Also, remove the configurations that users need to provide in the `InferenceService` (e.g., `HF_TOKEN`). These deployment-specific values (replica count, gateway binding, credentials) belong on the `InferenceService`, not the template.

**For example:**

```yaml {12} title="custom-prefill-dp16ep.yaml"
apiVersion: odin.moreh.io/v1alpha1
kind: InferenceServiceTemplate
metadata:
