...
2. Decoupling management: Utilizing a dedicated model manager pod to download model parameters and remote-code modules into the volume.
        ...
        - name: main
          ...
          args:
            - |
              ...
              && pip install huggingface_hub[cli] transformers --break-system-packages
...
For models that require `--trust-remote-code`, pre-populate the dynamic module cache on the shared volume before deploying the inference pods.

The model-manager pod above installs `transformers`, so you can warm the remote-code cache by resolving the model config and tokenizer before deploying the inference pods. Some models may still require additional packages such as `torch`, `peft`, or `accelerate` depending on their custom code.

```shell
hf download microsoft/Phi-mini-MoE-instruct
python3 - <<'PY'
...
print("remote code cached")
PY
```

This warm-up step resolves and caches the custom Python modules required by `trust_remote_code` models. In offline or air-gapped clusters, inference pods cannot fetch these modules at startup, so they must already exist on the shared volume.
