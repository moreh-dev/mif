---
title: DeepSeek R1 671B Maximum Throughput
sidebar_label: DeepSeek R1 Max Throughput
sidebar_position: 5
---
...
Please make sure to install all [prerequisites](../getting-started/prerequisites.mdx) before starting this benchmarking. Also, please refer to the [quickstart](../getting-started/quickstart.mdx) to understand how to run the MoAI Inference Framework.
...
**AWS credentials must be configured in this namespace to allow the container images of the MoAI Inference Framework to be downloaded**. For details, refer to the "Amazon ECR token for Moreh's container image repository" section in the [prerequisites](../getting-started/prerequisites.mdx).
...
```yaml #19 inference-service-values.yaml
global:
  imagePullSecrets:
    - name: moreh-registry
...
```yaml #5,15,26,28,44,47 benchmarking-client.yaml
apiVersion: v1
kind: Pod
metadata:
  annotations: {}
