...
```shell {2,7}
helm upgrade -i heimdall-llama moreh/heimdall-inference-scheduler \
    --version <heimdallVersion> \
    -n multi-model-test \
    -f heimdall-llama-values.yaml

helm upgrade -i heimdall-qwen moreh/heimdall-inference-scheduler \
    --version <heimdallVersion> \
    -n multi-model-test \
...
```shell
kubectl get pods -n multi-model-test -l app.kubernetes.io/name=heimdall-inference-scheduler
```
