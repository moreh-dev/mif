...
Enter a LogQL query in the query editor and click **Run query** (or press `Shift+Enter`). For example, `{app="aigateway"}` returns all logs from the AIGateway pods. Because AIGateway emits structured JSON, Vector parses each line and promotes its fields (such as `level`, `message`, and `target`), as the results below show:
