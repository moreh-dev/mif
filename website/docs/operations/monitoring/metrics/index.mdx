...
This document describes how to access Grafana dashboards to monitor the MoAI Inference Framework and provides an overview of the available metrics. Please make sure to install all [prerequisites](/getting-started/prerequisites.mdx) before starting this monitoring guide.
...
![MIF dashboard filters](./filters.png)
...
- **inference_pool**: Filter metrics by AIGateway name. The metrics are labeled with the name of the AIGateway routing the requests, so this filter scopes the view to a single gateway.
- **inference_service**: Filter metrics by inference service name. Use this to focus on specific model deployments.

By setting the **namespace** and **inference_pool** filters together, you can view metrics for a specific gateway. For example, if you followed the [quickstart guide](/getting-started/quickstart.mdx), you would set **namespace** to `quickstart` and **inference_pool** to `mif` (the AIGateway name) to see metrics for the `meta-llama/Llama-3.2-1B-Instruct` model deployed in that guide.

These filters work together to help you drill down into specific components of your infrastructure for detailed analysis. While you can typically use `All` for most filters to view aggregate metrics across your entire cluster, you may want to use specific filter values when comparing performance across different optimization options. For instance, if a gateway serves multiple inference services with different optimization configurations, you can use the **inference_service** filter to compare how each optimization option performs.
...
![MIF dashboard GPU total](./gpu-total.png)

### AMD / NVIDIA GPU

These sections provide detailed GPU metrics for each node type:

![MIF dashboard AMD / NVIDIA GPU](./gpu.png)
...
This section monitors the Heimdall gateway component:

![MIF dashboard Heimdall](./heimdall.png)

- **Ready Pods**: Shows the number of ready inference pods in each namespace and gateway combination.
- **Inference RPS**: Displays the requests per second (RPS) handled by the gateway for each namespace and gateway.
- **Heimdall E2EL**: Shows the end-to-end latency (E2EL) from the gateway's perspective, including P50, P75, and P95 percentiles. This metric measures the time from when a request enters the gateway until it is fully processed.
...
These metrics help you understand the gateway's routing performance and the efficiency of request routing.
...
![MIF dashboard InferenceService](./inferenceservice.png)
