...
These sections provide detailed per-node GPU metrics for each vendor. The dashboard detects AMD and NVIDIA GPU nodes automatically, so these panels populate without any filter selection.
