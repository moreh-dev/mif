...
- **hostname / amd_hostname / nvidia_hostname**: Filter the GPU sections by node. Selecting one or more GPU nodes scopes the GPU utilization, VRAM, and temperature panels to specific hosts.
...
These sections provide detailed per-node GPU metrics for each vendor. Select nodes with the **amd_hostname** or **nvidia_hostname** filters.

![MIF — AIGateway dashboard AMD GPU](./gpu.png)

- **GPU Utilization**: Utilization percentage for each GPU device on the selected nodes, displayed as both bar gauges and per-host time series graphs.
...
:::info
This section is only available when Odin is deployed and at least one InferenceService is running. If you do not see these metrics, ensure that an InferenceService has been created and is Ready.
:::
