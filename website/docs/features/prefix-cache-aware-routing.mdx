...
```yaml title="heimdall-values.yaml" {18}
config:
...
vLLM uses block hashes to look up prefixes. The scorer emulates this by hashing the tokenized prompt into blocks of size `B`. The `blockSize` must match the InferenceService's `--block-size` (the value vLLM is started with on the inference pods).
...
```yaml title="llama-prefix-cache.yaml" {19}
