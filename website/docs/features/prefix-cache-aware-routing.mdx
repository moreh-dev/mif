...
```yaml title="prefix-cache-profile.yaml" {12}
...
spec:
  ...
  plugins:
    - type: prefix-cache-scorer
      config:
        # blockSize is required and must match the engine's --block-size
        # (the value vLLM is started with on the inference pods).
        blockSize: 32
...
The `prefix-cache-scorer` accepts the following `config` fields:

- **`blockSize`** (required): the KV block size in tokens. It must match the engine's `--block-size`, otherwise the scorer's block hashes will not align with the engine's and no cache hits will be detected.
- **`normalization`** (optional, default `longestPrefix`): how the matched-block count is normalized into a `0`–`1` score. `longestPrefix` scores against the length of the matched prefix; `input` scores against the full input length.
- **`transform`** (optional, default `linear`): reshapes the normalized score before it enters the composite. `linear` is the identity; `logistic` applies a sigmoid (tunable via `k` and `x_0`) to sharpen the preference around a cache-hit threshold.
...
vLLM uses block hashes to look up prefixes. The scorer emulates this by hashing the request's tokens into blocks of size `B` (`blockSize`). The `blockSize` must match the InferenceService's `--block-size` (the value vLLM is started with on the inference pods); otherwise the scorer's block hashes will not align with the engine's and no cache hits will be detected.
...
The `mif.moreh.io/aigateway` label binds these pods to an `AIGateway` named `mif`. The `--block-size 32` value matches the `blockSize: 32` set in the `SchedulingProfile` above.
