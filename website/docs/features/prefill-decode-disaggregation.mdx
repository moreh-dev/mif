...
:::info
`pd-profile-handler` resolves both its decider plugin (default: `always-disagg-pd-decider`) and `disagg-headers-handler` at initialization, so both must appear **earlier** in the top-level `plugins` list. Swap in [`prefix-based-pd-decider`](../reference/heimdall/plugins.mdx#prefix-based-pd-decider) when you want prefill to run only after a cache-miss budget is exceeded. New deployments can also use the canonical [`disagg-profile-handler`](../reference/heimdall/plugins.mdx#disagg-profile-handler) with the nested `profiles.*`/`deciders.*` schema.
:::
