...
In the Heimdall configuration, you need to enable the [`pd-profile-handler`](../reference/heimdall/plugins.mdx#pd-profile-handler-legacy) plugin together with a [PD decider](../reference/heimdall/plugins.mdx#deciders) and [`disagg-headers-handler`](../reference/heimdall/plugins.mdx#disagg-headers-handler). The profile handler selects the appropriate prefill or decode pod using the `prefill` and `decode` scheduling profiles; the decider decides whether a given request needs prefill; and `disagg-headers-handler` writes the prefill endpoint into a request header so the decode pod can reach it. The [`prefill-filter`](../reference/heimdall/plugins.mdx#prefill-filter) and [`decode-filter`](../reference/heimdall/plugins.mdx#decode-filter) plugins filter pods by the `mif.moreh.io/role` label within their respective profiles. The configuration is part of the [`EndpointPickerConfig`](../reference/heimdall/api-reference.mdx#endpointpickerconfig).
...
config:
  plugins:
    # 1. Declare the PD decider and header handler first so pd-profile-handler can look them up during initialization
...
:::info
`pd-profile-handler` resolves both its decider plugin (default: `always-disagg-pd-decider`) and `disagg-headers-handler` at initialization, so both must appear **earlier** in the top-level `plugins` list.
:::

#### Using `disagg-profile-handler` (recommended for new deployments)

For new deployments, prefer [`disagg-profile-handler`](../reference/heimdall/plugins.mdx#disagg-profile-handler) over the legacy `pd-profile-handler`. It accepts a nested `profiles.*` / `deciders.*` schema and also manages an optional `encode` stage when you configure an encode decider — convenient for multimodal traffic.

Swap [`always-disagg-pd-decider`](../reference/heimdall/plugins.mdx#always-disagg-pd-decider) for [`prefix-based-pd-decider`](../reference/heimdall/plugins.mdx#prefix-based-pd-decider) when you want prefill to run only after the request exceeds a non-cached-token budget (set `nonCachedTokens` on the decider).
...
config:
  plugins:
    # 1. Declare the decider and header handler before the profile handler
    - type: disagg-headers-handler
    ...
    - type: queue-scorer
    - type: max-score-picker
    # 2. Canonical disagg profile handler wires them via nested parameters
...
:::info
The legacy flat parameter form (`decodeProfile`, `prefillProfile`, `deciderPluginName`) is still accepted by `disagg-profile-handler` for migrations from `pd-profile-handler`, but each legacy field emits a deprecation log entry. Use the nested form for new configs.
:::
...
```yaml title="deepseek-r1-decode.yaml" {29}
apiVersion: odin.moreh.io/v1alpha1
kind: InferenceService
metadata:
