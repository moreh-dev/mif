...
In the Heimdall configuration, you need to enable the [`pd-profile-handler`](../reference/heimdall/plugins.mdx#pd-profile-handler) plugin. This plugin selects the appropriate prefill or decode pod using the `prefill` and `decode` scheduling profiles. The [`prefill-filter`](../reference/heimdall/plugins.mdx#prefill-filter) and [`decode-filter`](../reference/heimdall/plugins.mdx#decode-filter) plugins filter pods by the `mif.moreh.io/role` label within their respective profiles. The configuration is part of the [`EndpointPickerConfig`](../reference/heimdall/api-reference.mdx#endpointpickerconfig).
...
In production or air-gapped environments, you can use **offline hub templates** (e.g., `vllm-hf-hub-offline`, `vllm-dp-hf-hub-offline`, or `vllm-pp-hf-hub-offline`) instead of `HF_TOKEN` to load models from a pre-provisioned Persistent Volume. These templates require a PVC named `models` in your namespace. Refer to the [Hugging Face model management with persistent volume](../operations/hf-model-management-with-pv.mdx) for more details.
:::
