...
In offline environments, also include the appropriate offline hub template in your `templateRefs` (e.g., `vllm-dp-hf-hub-offline` for DP setups, `vllm-pp-hf-hub-offline` for PP setups, or `vllm-hf-hub-offline` for TP-only setups).
:::
