...
:::warning
`pd-profile-handler` is legacy and has been replaced by [`disagg-profile-handler`](../reference/heimdall/plugins.mdx#disagg-profile-handler).
:::

In the Heimdall configuration, enable [`disagg-profile-handler`](../reference/heimdall/plugins.mdx#disagg-profile-handler) together with a [PD decider](../reference/heimdall/plugins.mdx#deciders) and [`disagg-headers-handler`](../reference/heimdall/plugins.mdx#disagg-headers-handler). The profile handler runs the `decode` profile first and then consults the configured decider to decide whether to also run `prefill`; when prefill is selected, `disagg-headers-handler` writes the prefill endpoint into a request header so the decode pod can reach it. The [`prefill-filter`](../reference/heimdall/plugins.mdx#prefill-filter) and [`decode-filter`](../reference/heimdall/plugins.mdx#decode-filter) plugins filter pods by the `mif.moreh.io/role` label within their respective profiles. The configuration is part of the [`EndpointPickerConfig`](../reference/heimdall/api-reference.mdx#endpointpickerconfig).

Consider the following `heimdall-values.yaml` example, which uses [`prefix-based-pd-decider`](../reference/heimdall/plugins.mdx#prefix-based-pd-decider) so prefill only runs when the request exceeds a non-cached-token budget. Swap it for [`always-disagg-pd-decider`](../reference/heimdall/plugins.mdx#always-disagg-pd-decider) if every request should run prefill.
...
config:
  plugins:
    # 1. Declare the decider and header handler before the profile handler so
    #    `disagg-profile-handler`'s factory can resolve them at init time.
    ...
    # 2. Wire the deciders via the nested parameter schema.
