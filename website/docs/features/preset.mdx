...
- **`ISVC_MODEL_PATH`**
  ...
  - Defaults to `spec.model.name`. When you set `ISVC_MODEL_PATH` to a local path, `spec.model.name` is still used as the served model name in other parts of the system (for example, in KV events).
- **`ISVC_EXTRA_ARGS`**
