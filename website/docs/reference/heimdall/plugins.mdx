...
Filters pods based on context length ranges defined by a pod label. Pods whose label-defined range does not cover the estimated token count of the request are removed.

This plugin also functions as a [scorer](#context-length-aware-1) — it can be registered once and used in both filter and scorer slots.
...
| `enableFiltering` | `bool`   | `false`                              | Whether to filter out pods that do not match the request's context length.                                 |
...
### `context-length-aware` {#context-length-aware-1}

Scores pods based on how well their context length range matches the estimated token count of the request. Pods with a matching range receive higher scores.

This plugin also functions as a [filter](#context-length-aware) — see the filter section for parameters.
