...
Filters for pods designated with the `prefill` role. It retains pods whose `mif.moreh.io/role` label value is one of `prefill`, `encode-prefill`, `prefill-decode`, `both`, or `encode-prefill-decode`. Pods without the role label are rejected.
...
- The label `mif.moreh.io/role` is set to `decode`, `prefill-decode`, `both`, or `encode-prefill-decode`.
...
| `idleThreshold`  | `int`     | `0`     | Maximum number of active requests for a pod to be considered idle. Pods at or below this count receive a score of `1.0`. |
| `maxBusyScore`   | `float64` | `1.0`   | Maximum score assigned to busy pods (those above `idleThreshold`). Setting below `1.0` creates a scoring gap between idle and busy pods; range `0.0`–`1.0`. |
