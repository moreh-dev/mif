...
| `prefixPluginName`  | `string` | `""`                        | Plugin name (instance name) of the prefix cache scorer.                                                                                  |
...
| `idleThreshold`  | `int`     | `0`     | Active-request count at or below which a pod is treated as idle and receives the maximum score.                                            |
| `maxBusyScore`   | `float64` | `1.0`   | Upper bound used when normalizing scores for non-idle pods. Pods at or above this load receive `0`; idle pods receive `1`.                  |
