...
Supported backends: in-memory or a Redis/Valkey-based tier with optional MongoDB tier-2 sync. Configure exactly one of `storeConfig.inMemoryConfig` or `storeConfig.tieredConfig` (Redis is required inside the tiered backend; MongoDB is optional).
