...
### `prefill-header-handler` (legacy)

Legacy alias of [`disagg-headers-handler`](#disagg-headers-handler). Both names register the same factory, so the parameters and behavior are identical. New deployments should use `disagg-headers-handler`; this name is retained only so existing `heimdall-values.yaml` files continue to work.
