...
Legacy alias for [`disagg-headers-handler`](#disagg-headers-handler) (both names share the same factory). Existing `heimdall-values.yaml` files that reference `prefill-header-handler` continue to work; new deployments should use `disagg-headers-handler`.
