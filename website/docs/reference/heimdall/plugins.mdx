...
| `maxBusyScore`   | `float64` | `1.0`   | Upper bound on the score assigned to busy pods (range `0.0`-`1.0`). Lower values widen the gap between idle and busy. |
...
| Parameter        | Type                            | Default | Description                                                              |
| :--------------- | :------------------------------ | :------ | :----------------------------------------------------------------------- |
| `inMemoryConfig` | [`Object`](#inmemoryconfig-1)   | -       | Configuration for the in-memory backend. Mutually exclusive with `tieredConfig`. |
| `tieredConfig`   | [`Object`](#tieredconfig)       | -       | Configuration for the Redis/Valkey + optional MongoDB tiered backend. Wins over `inMemoryConfig` when both are set. |
