...
- **Deciders** are helper plugins consumed by `disagg-profile-handler` (or its legacy alias `pd-profile-handler`) through its `deciders.*` parameters. They are declared in the top-level `plugins` list but **not** referenced from `schedulingProfiles`.
...
1. **Decode** always runs and selects the primary endpoint.
2. **Encode** (optional) runs first when the `deciders.encode` plugin decides the request benefits from a dedicated encode stage (for example, multimodal inputs).
3. **Prefill** (optional) runs after decode when the `deciders.prefill` plugin decides the request has enough uncached tokens to justify dedicated prefill.
...
Decider plugins are consumed by [`disagg-profile-handler`](#disagg-profile-handler) (or its legacy alias [`pd-profile-handler`](#pd-profile-handler-legacy)). Each decider answers one of two questions:

- **Prefill deciders** — "should this request run prefill?" Consumed via `deciders.prefill`.
- **Encode deciders** — "should this request run encode?" Consumed via `deciders.encode`.
...
:::warning
This scorer has a large parameter surface (20+ fields covering sampling, headroom weights, affinity gates, and selection strategies). Most deployments should leave every field at its default. Refer to the source at `pkg/epp/framework/plugins/scheduling/scorer/predictedlatency/scorer.go` in the GAIE fork for a complete list and tune only after establishing a baseline.
:::
...
Publishes the endpoints selected by `disagg-profile-handler` (or its legacy alias) as request headers, so the decode pod can reach prefill / encode pods:
...
:::info
`prefill-header-handler` is kept as a deprecated alias that resolves to this same plugin. Existing `heimdall-values.yaml` files that reference `prefill-header-handler` continue to work.
:::
