...
Score every candidate `1.0`, regardless of metrics or context.
...
Score by GPU KV cache occupancy: workers whose cache holds fewer reusable cached blocks score higher.

### `kv-utilization-scorer`

Score by KV cache memory utilization.
...
Scores candidates by how well their advertised `optimal_min` matches the prompt length, max-normalized so the best candidate is `1.0`.
