...
- **Data layer plugins** (sources and extractors) feed pod metrics into the scheduler. They are referenced from the top-level [`data`](./api-reference.mdx#datalayerconfig) field, not from `schedulingProfiles`, and require the `dataLayer` feature gate to be enabled.
...
For a full walkthrough, see [PD disaggregation](../../features/prefill-decode-disaggregation.mdx).
...
Profile handler for data-parallel deployments. Expects exactly one `SchedulingProfile`; when configured with anything other than one, the handler returns an empty selection. After picking the primary endpoint, the handler optionally rewrites the endpoint's port to `primaryPort`, which is useful when a sidecar (such as Istio) terminates the data-parallel split on a different port from the model server.

| Parameter     | Type  | Default | Description                                                                                                                                       |
| :------------ | :---- | :------ | :------------------------------------------------------------------------------------------------------------------------------------------------ |
| `primaryPort` | `int` | `8000`  | Port to substitute on the selected endpoint. Set to `0` to keep the endpoint's original port. Must be between `1` and `65535` when non-zero.       |

:::info
On startup, this handler logs a deprecation notice recommending `single-profile-handler` paired with Istio 1.28.1 or newer. New deployments should follow that recommendation; this handler is retained for clusters that cannot run a recent enough Istio.
:::
...
### `prefill-weight-filter` / `decode-weight-filter`

Splits candidate endpoints between the prefill and decode phases according to a configured worker-weight ratio. Both filters share the same factory and parameters; the type name (`prefill-weight-filter` vs `decode-weight-filter`) determines which side of the split each instance keeps.

| Parameter  | Type      | Default | Description                                                                                                                                                                                                       |
| :--------- | :-------- | :------ | :---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `weight`   | `float64` | `1.0`   | Worker weight assigned to this side of the split. Must be greater than `0`. The prefill/decode counts are derived from the ratio of the two filters' weights. Initialization fails if `weight` is `0` or negative. |
| `precise`  | `bool`    | `false` | When `false`, the filter splits the candidate list by ratio (the first N go to prefill, the rest to decode). When `true`, endpoints are bucketed per role using a shared worker cache.                              |

Both filters must be instantiated together (one of each type) for the split to work; using only one yields no candidates for the other phase.
...
| `idleThreshold`  | `int`     | `0`     | Maximum active-request count at which a pod is still considered idle. Idle pods receive a score of `1.0`. With the default `0`, only pods with zero in-flight requests are idle.                                                            |
| `maxBusyScore`   | `float64` | `1.0`   | Cap on the score assigned to busy pods (those above `idleThreshold`). Lowering this value widens the gap between idle and busy pods. With the default `1.0`, busy pods can score up to `1.0`; with `0.5`, the busiest pod scores `0.0` and the least busy busy pod scores `0.5`. |
...
Supported backends: an in-memory store, or a tiered store with a hot Redis tier and an optional cold MongoDB tier. Configure exactly one of `storeConfig.inMemoryConfig` or `storeConfig.tieredConfig`; if both are set, `tieredConfig` wins.
...
The `storeConfig` object exposes two backend sub-objects:

- `inMemoryConfig` — process-local store with `ttl` (Go duration, default `"24h"`), `maxEntries`, and `maxEntryBytes`.
- `tieredConfig` — a hot Redis tier with an optional cold MongoDB tier, configured under `tieredConfig.redis` and `tieredConfig.mongo`. When `mongo` is omitted, the store runs in Redis-only (tier-1) mode. An optional `tieredConfig.stream` block tunes the Redis Streams consumer that replicates entries from Redis to MongoDB. Each backend tier carries its own TTL on its config block (for example, `tieredConfig.redis.ttl`).

Use the in-memory backend for smoke testing and the tiered backend for production. Standalone Valkey configuration was removed; deploy a Valkey-compatible service through `tieredConfig.redis` instead.
