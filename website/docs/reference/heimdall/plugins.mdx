...
| `idleThreshold`  | `int`     | `0`     | Number of in-flight requests below which a pod is considered idle. Idle pods receive `maxBusyScore` regardless of their absolute count. |
| `maxBusyScore`   | `float64` | `1.0`   | Score assigned to idle pods (and the upper bound used when normalizing busy pods).                    |
