...
:::warning
If the AMD GPU driver is already installed on the host OS, set `driver.enable` to `false` to skip driver installation by the GPU Operator.
:::

:::info
When installing the GPU driver via the GPU Operator, refer to the [User-space and kernel-space compatibility matrix](https://rocm.docs.amd.com/projects/install-on-linux/en/latest/reference/user-kernel-space-compat-matrix.html) to select a `driver.version` that is compatible with the host OS kernel.
:::
