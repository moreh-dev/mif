...
```yaml title="moai-inference-framework-values.yaml" {4-5,10,16,19}
...
:::info
`<storage-class>` must match a StorageClass available in your cluster. Run `kubectl get storageclass` to list available StorageClasses. If omitted, the cluster's default StorageClass is used; if no default exists, the PVC will remain in `Pending` state.
:::
