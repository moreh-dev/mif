...
:::info
In environments with multiple Istio control planes, you may need to specify an Istio revision for the Gateway by adding the `istio.io/rev: <revision>` label to the `Gateway` metadata.
:::

```yaml title="gateway.yaml" {41}
apiVersion: v1
kind: ConfigMap
metadata:
...
```shell title="Expected output (kubectl get pod -n quickstart -l gateway.networking.k8s.io/gateway-name=mif)"
...
```yaml title="heimdall-values.yaml" {25}
global:
  imagePullSecrets:
    - name: moreh-registry
...
```shell
kubectl get all -n quickstart -l app.kubernetes.io/instance=heimdall-inference-scheduler
```

Expected output:

```shell title="Expected output (kubectl get all -n quickstart -l app.kubernetes.io/instance=heimdall-inference-scheduler)"
NAME                            READY   STATUS    RESTARTS   AGE
pod/heimdall-inference-scheduler-7d54fcbfff-chw94   1/1     Running   0          70s

NAME               TYPE        CLUSTER-IP     EXTERNAL-IP   PORT(S)                      AGE
service/heimdall-inference-scheduler   ClusterIP   10.110.35.57   <none>        9002/TCP,9090/TCP,5557/TCP   70s

NAME                       READY   UP-TO-DATE   AVAILABLE   AGE
deployment.apps/heimdall-inference-scheduler   1/1     1            1           70s

NAME                                  DESIRED   CURRENT   READY   AGE
replicaset.apps/heimdall-inference-scheduler-7d54fcbfff   1         1         1       70s
```
...
```yaml title="vllm-llama3-1b-instruct-tp2.yaml" {18}
...
```shell title="Expected output (kubectl get all -n quickstart -l app.kubernetes.io/name=vllm-llama3-1b-instruct-tp2)"
NAME                                               READY   STATUS    RESTARTS   AGE
...
helm uninstall -n quickstart heimdall-inference-scheduler
