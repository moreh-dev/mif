...
- **InferenceService** (Odin): a collection of vLLM pods running the model across GPUs/servers. Each pod is bound to an AIGateway by a label.
...
graph TB
    ...
    subgraph odin["Odin InferenceService"]
      vllm1[vLLM 1]
      vllm2[vLLM 2]
    end
    gateway --> vllm1
