...
Each consumer's secret key defaults to a generated random value rather than a
static password (`mif.bucketSecretKey` helper): `<consumer>Bucket.secretKey` if
set, else the value already in the `<consumer>-bucket` Secret (preserved across
upgrades via `lookup`), else `randAlphaNum` on first install. The access key
stays the static username (`loki`/`tempo`).
...
| `templates/loki/credentials.yaml` | `AWS_ACCESS_KEY_ID` defaults to the dedicated `loki` user; `AWS_SECRET_ACCESS_KEY` to a generated random value (preserved across upgrades via `lookup`, overridable) — no longer `minio.rootUser`/`rootPassword`. |
...
| `templates/tempo/credentials.yaml` | `tempo-bucket` ConfigMap (`BUCKET_*`) + Secret (`AWS_*`); access key `tempo`, secret key a generated random value (overridable). Mirrors `templates/loki/credentials.yaml`. |
